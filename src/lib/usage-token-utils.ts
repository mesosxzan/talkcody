...
export function normalizeUsageTokens(
  ...
  if (totalTokens > 0 && (inputTokens > 0 || outputTokens > 0)) {
    // Include cache tokens in total to reflect actual context window usage.
    // Cache tokens (read + creation) occupy the same context window as regular input tokens,
    // so they must be counted for accurate context pressure detection.
    totalTokens =
      inputTokens + outputTokens + (cachedInputTokens ?? 0) + (cacheCreationInputTokens ?? 0);
  }
