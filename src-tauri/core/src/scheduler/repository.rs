        ...
        self.db
            .execute(
                r#"INSERT INTO scheduled_tasks (
                    ...
                    payload_message, payload_model, payload_agent_id, payload_auto_approve_edits, payload_auto_approve_plan,
                ...
                vec![
                    ...
                    serde_json::json!(task.payload.agent_id),
        ...
        self.db
            .execute(
                r#"UPDATE scheduled_tasks SET
                    ...
                    payload_message = ?, payload_model = ?, payload_agent_id = ?,
...
fn row_to_task(row: &serde_json::Value) -> Result<ScheduledTask, String> {
    ...
    Ok(ScheduledTask {
        ...
        payload: ScheduledTaskPayload {
            ...
            agent_id: row["payload_agent_id"].as_str().map(|s| s.to_string()),
